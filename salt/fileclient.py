...
class Client(object):
    ...
    def file_list(self, env='base', prefix=''):  # pylint: disable=W0613
        '''
        This function must be overwritten
        '''
        return []

    def dir_list(self, env='base', prefix=''):  # pylint: disable=W0613
        '''
        This function must be overwritten
        '''
        return []
...
class LocalClient(Client):
    ...
    def file_list(self, env='base', prefix=''):
        ...
        for path in self.opts['file_roots'][env]:
            for root, dirs, files in os.walk(os.path.join(path, prefix), followlinks=True):  # pylint: disable=W0612
                for fname in files:
                    ret.append(
                        os.path.relpath(
