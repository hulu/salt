...
def _verify_gitpython(quiet=False):
    ...
    if not HAS_GITPYTHON:
        ...
        return False
    gitver = distutils.version.LooseVersion(git.__version__)  # pylint: disable=E0611,E1101
    minver_str = '0.3.0'
    minver = distutils.version.LooseVersion(minver_str)  # pylint: disable=E0611,E1101
    errors = []
    if gitver < minver:
        errors.append(
            'Git fileserver backend is enabled in master config file, but '
...
def _verify_pygit2(quiet=False):
    ...
    pygit2ver = distutils.version.LooseVersion(pygit2.__version__)  # pylint: disable=E0611,E1101
    pygit2_minver_str = '0.20.3'
    pygit2_minver = distutils.version.LooseVersion(pygit2_minver_str)  # pylint: disable=E0611,E1101

    libgit2ver = distutils.version.LooseVersion(pygit2.LIBGIT2_VERSION)  # pylint: disable=E0611,E1101
    libgit2_minver_str = '0.20.0'
    libgit2_minver = distutils.version.LooseVersion(libgit2_minver_str)  # pylint: disable=E0611,E1101
...
def _file_list_gitpython(repo, tgt_env):
    ...
    for file_blob in tree.traverse():  # pylint: disable=E1103
